...
後はログに記録させればいいので、なんらかのアクセス情報か入力にそのアドレスを入れ込んでおけば、真面目なサーバならそれを勝手に記録してくれるという寸法である。攻撃するほうは条件だけ整えたらあとは単にアクセスしまくるだけで攻撃できてしまう。
...
実際にどう攻撃するのか？今回 `Log4j2` の何が不味かったか？
...
できればリリースしたくない問題
--------------------------------------------------------------------------------
このような単純な末端のプログラムに多数の機能を持たせてしまう原因として、
根幹部分を変更したくない圧力があると思われる。

根幹のプログラムを変更するならば、システムのリリースが必要でリリースには仕様の決定が必要で、それに対する実装、テスト、デプロイと必要になる。些細な変更だとしてもプロジェクトの腰が重くなる。

なぜ腰が重くなるのか、表向きはコストの問題で片付けられるが実際は、みんな自分の能力がばれるのが嫌だからだ。

- 仕様を変更すると影響がでかいのでやらない → 影響を考慮し思慮深く仕様を決定する能力が無い
- 時間がかかるので実装は難しい → 仕様を理解する能力も、それを吟味する能力も、そもそも実装する能力も無い
- テストはコストがかかるだけで効果が薄い → テストを作る能力も無いし、そもそも作ったことすら無い
- 以上の過程があるのでデプロイ回数を減らしたほうがいい

こうなると、プログラムの変更を仕様変更とは言えないような些細な処理として済ませたいという気持ちが働く。

- 些細な変更なので深く考えて仕様を決めるとか無い
- プログラムの変更ではなくデータの変更、設定の変更にすればいい
- プログラムの変更が無いのでテストしなくていい
- プログラムの変更が無いのでデプロイが必要無い

こうなると、ロギングのような末端の外部プログラムに機能のしわ寄せが来る。
プログラムの変更無くパラメータでなんかとかできるようにしたりとか、ほとんど必要無いような危ない機能を追加したりするようになる。

今回の件もそのようなプロジェクトの人間からの些細なリクエストから実装した機能に問題があったという話だ。
